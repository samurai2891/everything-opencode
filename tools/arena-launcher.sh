#!/bin/bash
# =============================================================================
# Arena Launcher - 一気通貫でtmuxセッションを起動し、複数LLMエージェントを並列実行
# =============================================================================
# 使用方法:
#   arena-launcher.sh "要件テキスト"
#   arena-launcher.sh --file requirements.md
#   arena-launcher.sh --teams 3 "要件テキスト"
# =============================================================================
...
show_help() {
    cat << EOF
Arena Launcher - 複数LLMエージェントを並列実行するtmuxセッションを起動

使用方法:
  arena-launcher.sh [オプション] "要件テキスト"
...
例:
  arena-launcher.sh "ECサイトのカート機能を実装してください"
  arena-launcher.sh -t 5 -f requirements.md
  arena-launcher.sh --teams 3 --session my-arena "3Dゲームを作成"
...
create_arena_session() {
    ...
    # 2. Central Plannerを起動
    local planner_prompt="以下の要件に基づいてアリーナ競争を開始してください。各チームにタスクを割り当て、進捗を監視してください。
...
4. 最終的な統合を指示する"
    ...
    sleep 1
    tmux send-keys -t "$SESSION_NAME:planner" "$planner_prompt" Enter
    ...
    for i in "${!team_names[@]}"; do
        if [[ $i -lt $NUM_TEAMS ]]; then
            ...
            local team_prompt="あなたは競争チーム $team（$role担当）です。
...
実装を開始してください。"
            ...
            sleep 0.5
            tmux send-keys -t "$SESSION_NAME:$team" "$team_prompt" Enter
    ...
    local qa_prompt="あなたはQA Gateです。各チームの実装を評価してください。
...
評価を開始してください。"
    ...
    sleep 0.5
    tmux send-keys -t "$SESSION_NAME:qa-gate" "$qa_prompt" Enter
    log_success "QA Gateを起動しました"
    
    # 5. Integratorウィンドウを作成
    tmux new-window -t "$SESSION_NAME" -n "integrator"
    local integrator_prompt="あなたはIntegratorです。勝者の実装を統合してください。
...
統合準備を開始してください。"
    ...
    sleep 0.5
    tmux send-keys -t "$SESSION_NAME:integrator" "$integrator_prompt" Enter
...
main() {
    echo ""
    echo "=============================================="
    echo "       Arena Launcher - 並列LLM競争システム"
    echo "=============================================="
    echo ""
